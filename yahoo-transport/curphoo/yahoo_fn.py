...
IDENT  = 1 # identify function
XOR    = 2 # xor with arg1
MULADD = 3 # multipy by arg1 then add arg2
LOOKUP = 4 # lookup each byte in the table pointed to by arg1
BITFLD = 5 # reorder bits according to table pointed to by arg1
...
def A( x ): return (( x ) & 0xFF )
def B( x ): return (( x ) >> 8 & 0xFF )
def C( x ): return (( x ) >> 16 & 0xFF )
def D( x ): return (( x ) >> 24 & 0xFF )

def yahoo_xfrm(table, depth, seed):
    n = seed
